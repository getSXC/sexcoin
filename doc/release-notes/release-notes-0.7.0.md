...
We recommend that everybody running prior versions of sexcoind/Sexcoin-qt
...
in your terminal, then install the sexcoin-qt package:
  sudo apt-get install sexcoin-qt
...
* (Windows only): enable ASLR and DEP for sexcoin-qt.exe
* (Windows only): add meta-data to sexcoin-qt.exe (e.g. description)
...
* Bash programmable completion for sexcoind(1)
