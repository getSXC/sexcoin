...
Commands sent over the JSON-RPC interface and through the `sexcoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`sexcoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/sexcoin-cli -named help command="help"
    src/sexcoin-cli -named getblockhash height=0
    src/sexcoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/sexcoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
- #8722 `bb843ad` sexcoin-cli: More detailed error reporting (laanwj)
...
- #8813 `fb24d7e` sexcoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow sexcoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` sexcoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to sexcoin-tx (jnewbery)
