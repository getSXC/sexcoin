...
- #7550 `8b958ab` Input-from-stdin mode for sexcoin-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to sexcoin-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: sexcoin-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from sexcoind (sipa)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for sexcoind procs (MarcoFalke)
...
- #8164 `0f24eaf` [Sexcoin-tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of sexcoind startup (laanwj)
