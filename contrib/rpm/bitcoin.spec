...
#man pages
Source20:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/sexcoind.1
Source21:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/sexcoin-cli.1
Source22:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/sexcoin-qt.1

#selinux
Source30:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/contrib/rpm/bitcoin.te
# Source31 - what about sexcoin-tx and bench_bitcoin ???
...
The sexcoin-cli utility allows you to communicate and control a bitcoin daemon
over RPC, the sexcoin-tx utility allows you to create a custom transaction, and
...
mv %{buildroot}%{_bindir}/sexcoind %{buildroot}%{_sbindir}/sexcoind

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoin.conf
d /run/sexcoind 0750 bitcoin bitcoin -
EOF
...
PID_FILE="/run/sexcoind/sexcoind.pid"
EOF
...
[Service]
Type=forking
ExecStart=%{_sbindir}/sexcoind -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
...
%{_bindir}/inkscape %{SOURCE100} --export-png=%{buildroot}%{_datadir}/pixmaps/sexcoin128.png -w128 -h128
...
%{_bindir}/convert -resize 128x128 %{buildroot}%{_datadir}/pixmaps/bitcoin256.png %{buildroot}%{_datadir}/pixmaps/sexcoin128.xpm
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/sexcoind.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/sexcoin-cli.1
%if %{_buildqt}
install -p %{SOURCE22} %{buildroot}%{_mandir}/man1/sexcoin-qt.1
%endif
...
%attr(0755,root,root) %{_bindir}/sexcoin-qt
...
%attr(0644,root,root) %{_mandir}/man1/sexcoin-qt.1*
%endif
...
%attr(0755,root,root) %{_sbindir}/sexcoind
...
%attr(0644,root,root) %{_mandir}/man1/sexcoind.1*

%files utils
%defattr(-,root,root,-)
%license COPYING
%doc COPYING bitcoin.conf.example doc/README.md
%attr(0755,root,root) %{_bindir}/sexcoin-cli
%attr(0755,root,root) %{_bindir}/sexcoin-tx
%attr(0755,root,root) %{_bindir}/bench_bitcoin
%attr(0644,root,root) %{_mandir}/man1/sexcoin-cli.1*
