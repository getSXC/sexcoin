...
def main():
    ...
    enable_sexcoind = config["components"].getboolean("ENABLE_sexcoind")
    ...
    if not (enable_wallet and enable_utils and enable_sexcoind):
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if sexcoind is already running (unix only)
    try:
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie sexcoinds, we can apply a
...
class RPCCoverage(object):
    ...
    commands per `sexcoin-cli help` (`rpc_interface.txt`).
