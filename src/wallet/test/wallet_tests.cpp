    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see sexcoind log for details) and could be dealt with by "
...
public:
    ...
    CWalletTx& AddTx(CRecipient recipient)
    {
        ...
        std::string error;
        CCoinControl dummy;
        BOOST_CHECK(wallet->CreateTransaction({recipient}, wtx, CTransaction::CURRENT_VERSION, reservekey, fee, changePos, error, dummy));
